...
namespace crsm_slam
{
  ...
  CrsmSlam::CrsmSlam(int argc, char **argv){
    ...
    _pathPublishingTimer = n.createTimer(
      ros::Duration(1.0 / slamParams.trajectory_freq), 
      &CrsmSlam::publishTrajectory, this, false, false);

    _robotPosePublishingTimer = n.createTimer(
      ros::Duration(1.0 / slamParams.robot_pose_tf_freq), 
      &CrsmSlam::publishRobotPoseTf, this, false, false);

    _robotPosePublishingTimer.start();

    _mapPublishingTimer = n.createTimer(
      ros::Duration(1.0 / slamParams.occupancy_grid_map_freq),
  ...
  void CrsmSlam::updateParameters(void)
  {
    ...
    {
      ROS_WARN("[CrsmSlam] : Parameter slam_container_size not found. \
        Using Default");
    ...
    {
      ROS_WARN("[CrsmSlam] : Parameter map_update_density not found.\
        Using Default");
    ...
    try 
    {
      _listener.waitForTransform(slamParams.base_frame, slamParams.laser_frame, 
        ros::Time(0), ros::Duration(1));
      
      _listener.lookupTransform(slamParams.base_frame, slamParams.laser_frame, 
  ...
  void CrsmSlam::findTransformation(void){
    ...
    while(!isDone) 
    {
        ...
        j != scanSelections.end() ; j++)
      {
        ...
        if(checkExpansion(tttx, ttty, false))
        {
          continue;
        }
      ...
      else
      {
        ...
        trier.t.dy = 
          rand() % slamParams.disparity / 2 - slamParams.disparity / 4;
        trier.t.dth = 
          (rand() % slamParams.disparity - slamParams.disparity / 2.0) / 45.0;
      }
  ...
  void CrsmSlam::calculateCriticalRays(void)
  {
      ...
      j != bigChanges.end() ; )
    {
      ...
      for(unsigned int i = start ; i < end ; i++)
      {
        ...
        if( localPos > (parameter * step / 
           exp(laser.scan.distance[i] / sqrt(maxRay) * 3.0)))
        {	
  ...
  void CrsmSlam::fixNewScans(const sensor_msgs::LaserScanConstPtr& msg)
  {
    ...
    for(unsigned int j = 0 ; j < laser.info.laserRays ; j++) 
    {
      ...
      laser.scan.p[j].x = laser.scan.distance[j] / slamParams.ocgd * 
        cos(laser.scan.p[j].theta);
      ...
      it != scanSelections.end() ; it++)
    {
      if(laser.scan.distance[*it] == 0 || 
        laser.scan.distance[*it] == laser.info.laserMax)
      {
    ...
    if (robotTrajectory.size() == 0 ||
      robotTrajectory[robotTrajectory.size() - 1].x != 
      (robotPose.x - cos(robotPose.theta) * 
       slamParams.dx_laser_robotCenter / slamParams.ocgd) || 
      robotTrajectory[robotTrajectory.size() - 1].y != 
      (robotPose.y - sin(robotPose.theta) * 
       slamParams.dx_laser_robotCenter  /slamParams.ocgd))
    {
      trajectoryTemp.x = robotPose.x - 
        cos(robotPose.theta) * 
        slamParams.dx_laser_robotCenter / slamParams.ocgd;
      
      trajectoryTemp.y = robotPose.y - 
        sin(robotPose.theta) * 
        slamParams.dx_laser_robotCenter / slamParams.ocgd;
  ...
  void CrsmSlam::updateMapProbabilities(void)
  {
    ...
    checkExpansion(	
      robotPose.x + map.info.originx - laser.info.laserMax / slamParams.ocgd,
      robotPose.y + map.info.originy - laser.info.laserMax / slamParams.ocgd,
      true);
    checkExpansion(	
      robotPose.x + map.info.originx + laser.info.laserMax / slamParams.ocgd,
      robotPose.y + map.info.originy + laser.info.laserMax / slamParams.ocgd,
    ...
    for(unsigned int i = 0 ; i < laser.info.laserRays ; i++)
    {
      ...
      float yPoint;
      xPoint = 
        laser.scan.distance[i] / slamParams.ocgd * 
        cos(robotPose.theta + 
          (float)i / (float)laser.info.laserRays * 
          laser.info.laserAngle + laser.info.laserAngleBegin) + 
        robotPose.x + map.info.originx;
      
      yPoint = laser.scan.distance[i] / slamParams.ocgd * 
        sin(robotPose.theta + (float)i / (float)laser.info.laserRays * 
          laser.info.laserAngle + laser.info.laserAngleBegin) + 
    ...
    for(unsigned int measid = 0 ; measid < laser.info.laserRays ; measid += 1)
    {
      ...
      while(R < laser.info.laserMax / slamParams.ocgd - 3)
      {
        ...
        if(checkExpansion((int)xPoint, (int)yPoint, false))
        {
          break;
        }
  ...
  /**
    ...
    @return void
   **/
  void CrsmSlam::startLaserSubscriber(void)
  {
    clientLaserValues = 
      n.subscribe( slamParams.laser_subscriber_topic.c_str() , 1 , 
        &CrsmSlam::fixNewScans , this );
  }
  ...
  /**
    ...
    @return void
   **/
  void CrsmSlam::stopLaserSubscriber(void)
  {
  ...
  /**
    ...
    @return void
   **/
  void CrsmSlam::startOGMPublisher(void)
  {
    _occupancyGridPublisher = n.advertise<nav_msgs::OccupancyGrid>
  ...
  /**
    ...
    @return void
   **/
  void CrsmSlam::startTrajectoryPublisher(void)
  {
  ...
  /**
    ...
    @return double [OC]cupancy [G]rid [D]imentionality - the width and height 
    in meters of a pixel
   **/
